//This Function stores data of all the age group question.
...
// Function to display questions- this function will reset index and selectedanswer from the start and call another function displaycurrentquestion where quiz question 
//function is passed as a first parameter and currentquestionindex as a second parameter.
...
//Displays the current question in a quiz interface.
  // This function updates the DOM with the current question and its answers based on the provided index.
 // It handles user interactions by:
 //- Showing the next button after an answer is selected.
  //- Advancing to the next question or displaying results when appropriate.
  //@param {Array} questions - An array of question objects, each with a question, answers, and points.
...
//Displays the results of the quiz upon completion.
//This function hides the quiz interface and reveals the end screen. It calculates the user's score
...
// When the start button is clicked, it checks if all fields, including the child's name, age group, and diet type, are filled. 
// It validates the diet type to ensure it’s either "veg" or "non-veg." 
//If all inputs are valid, the code calls a function to load questions based on the age group
//and diet type, hiding the quiz info section and displaying the quiz.
// The "next" button saves the age group to local storage
//and fades out before redirecting to the diet plan page.
let userAgeGroup;
...
//This code customizes the diet plan page based on the user's age group from localStorage.
//On page load, it updates diet images, descriptions, water intake, and exercise advice according to age
//For specific age groups,
//it adds a second image to show both male and female diet options. 
//If any elements are missing, an error is logged.
document.addEventListener('DOMContentLoaded', () => {
  if (window.location.href.includes('dietplan.html')) {
    ...
    if (dietImage && dietDescription && waterText && exerciseText) {
      ...
      } else if (storedUserAgeGroup === '18-60 years') { // Removed space between 18 and -
